...
CSV.open('fullconact_developer_affiliation.csv', 'w') do |csv|
  ...
  email_list.each do |email_with_exclamation|
    ...
    begin
      ...
      raise person.message if person.status != 200
      # binding.pry
      orgs = []
      org_cnt = 1
      org_details = ''
      ...
      if !person&.organizations.nil?
        person.organizations.each do |organization|
          organization.is_primary ? (org_details += 'primary|') :
                                  (org_details += 'secondary|')
          organization.current ? (org_details += 'current|') :
                               (org_details += 'past|')
          org_details += "#{organization.start_date}|"
          org_details += "#{organization.end_date}|"
          org_details += organization.title
      ...
      if hash.index('FullContact_lookup_failed')
        csv << ['error', 'bad', 'email', email_with_exclamation]
        bad_cnt += 1
        puts "#{check_cnt} received a bad email msg"
      elsif hash.index('Usage limits for the provided API Key have been exceeded.')
        csv << ['error', 'usage limit', 'exceeded', email_with_exclamation]
        bad_cnt += 1
        puts "#{check_cnt} received a bad email msg"
      else
